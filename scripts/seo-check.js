...
function checkContent(filePath, pattern, description) {
  try {
    ...
    if (pattern.test(content)) {
      ...
      checks.warnings.push(`⚠️  ${description} - Pattern not found in ${filePath}`);
      return false;
    }
  } catch (error) {
...
checkContent(indexPath, /<meta property="og:title"/, 'Open Graph title present');
checkContent(indexPath, /<meta property="og:description"/, 'Open Graph description present');
checkContent(indexPath, /<meta property="og:image"/, 'Open Graph image present');
...
checkContent(indexPath, /"@type":\s*"WebApplication"/, 'Schema.org WebApplication structured data');
checkContent(indexPath, /"@type":\s*"Organization"/, 'Schema.org Organization structured data');
checkContent(indexPath, /"@type":\s*"BreadcrumbList"/, 'Schema.org BreadcrumbList structured data');
...
checkContent(sitemapPath, /https:\/\/diagrammatic\.next-zen\.dev/, 'Sitemap uses correct domain');
