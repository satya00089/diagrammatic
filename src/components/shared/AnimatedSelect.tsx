...
const AnimatedSelect: React.FC<AnimatedSelectProps> = ({ id, label, value, onChange, options, disabled, className='', containerClassName }) => {
  const { focused, onFocus, onBlur } = useFocus();
  const labelId = `${id}-label`;
  return (
    <FieldWrapper id={id} label={label} focused={focused} containerClassName={containerClassName}>
      {!label && <label id={labelId} htmlFor={id} className="sr-only">{id}</label>}
      <motion.div
