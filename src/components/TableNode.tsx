...
const TableNode: React.FC<Props> = React.memo(({ id, data, onCopy, isInGroup }) => {
  ...
  const renderColumnCell = useCallback(
    (col: typeof columns[0], attr: TableAttribute, isEditing: boolean) => {
      ...
      if (col.type === "boolean") {
        ...
        const colorClass = displayValue ? col.color?.active : col.color?.inactive;
      ...
      if (col.type === "text" && col.editable) {
        if (isEditing) {
          return (
            <input
              key={col.key}
              type="text"
              value={String(editingValue[col.key as keyof typeof editingValue] || attrValue || "")}
              onChange={(e) =>
                setEditingValue({ ...editingValue, [col.key]: e.target.value })
              }
              onKeyDown={(e) => {
        ...
        if (col.key === "name") {
          if (attr.isPrimaryKey) {
            textColorClass += " font-semibold text-yellow-600 dark:text-yellow-400";
      ...
      // Default fallback
      return <span key={col.key} className={cellClassName}>{String(attrValue || "")}</span>;
    },
    [
      handleToggleBoolean,
